...
namespace Facturas
{
    public partial class Form1 : Form
    {
        ...
        private void btnIngresar_Click(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(txtCantJ.Text, out int cantidad)) { MessageBox.Show("Cantidad debe ser un número entero.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }

            if (!double.TryParse(txtPrecioU.Text, out double precioU)) { MessageBox.Show("Precio Unitario debe ser un número.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }

            if (!int.TryParse(txtNumTelefonico.Text, out int telefono)) { MessageBox.Show("Teléfono debe ser un número entero.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }

            if (!int.TryParse(txtCodigo.Text, out int codigo)) { MessageBox.Show("Código debe ser un número entero.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error); return; }
            ...
            for (int i = 0; i < nombreList.Count; i++)
            {
                ...
                {
                    MessageBox.Show("El código ingresado ya se encuentra registrado", "Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            }
        ...
        private int obtenerCodigoValido()
        {
            while (true)
            {
                string input = Microsoft.VisualBasic.Interaction.InputBox("Ingrese el código del producto:", "Código del Producto", "");

                if (string.IsNullOrWhiteSpace(input))
                {
                    MessageBox.Show("El codigo no puede estar vacio.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    return -1; // Indica que no se ingresó un código válido
                }
                ...
                if(int.TryParse(input, out codigoBuscar))
                {
                    ...
                    {
                        return codigoBuscar; // Retorna el código válido
                    }
                }
                ...
                if(!int.TryParse(input, out codigoBuscar))
                {
                    ...
                    continue; // Vuelve a solicitar el código
                }
